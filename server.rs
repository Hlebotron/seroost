...
fn serve_request(model: Arc<Mutex<Model>>, request: Request, search_dir: &String) -> io::Result<()> {
    println!("INFO: received request! method: {:?}, url: {:?}", request.method(), request.url());
    let mut file_path: &str = "${HOME}/.config/i3/config";
    let mut extension: &str = "txt";
    let url = if let true = request.url().contains(search_dir) {
        file_path = request.url(); 
        extension = request.url().split(".").last().unwrap();
        "file"
    } else {
        request.url()
    };
    match (request.method(), url) {
        ...
        (Method::Get, "file") => {
            let mut format = format!("text/{extension}; charset=utf-8");
            let mut file = File::open(file_path)?;
            let mut file_bytes = Vec::new();
            file.read_to_end(&mut file_bytes)?;
            serve_bytes(request, &file_bytes, &format)
...
pub fn start(address: &str, model: Arc<Mutex<Model>>, search_dir: String) -> Result<(), ()> {
    ...
    for request in server.incoming_requests() {
        serve_request(Arc::clone(&model), request, &search_dir).map_err(|err| {
